...
type Props = { rating: number; starSize?: number };

const DEFAULT_STAR_SIZE = 24;

const RatingStars = ({ rating, starSize = DEFAULT_STAR_SIZE }: Props) => {
    const starRating = Math.round(rating * 2) / 2;
    const fullStars = Math.floor(starRating);
    const halfStars = starRating - fullStars >= 0.5;
    const emptyStars = 5 - fullStars - (halfStars ? 1 : 0);
    return (
        <div className="flex items-center gap-0.5">
            {[...Array(fullStars)].map((_, index) => (
                <svg key={`full-star-${index}`} width={starSize} height={starSize} viewBox="0 0 20 20">
                    <path
            ...
            {emptyStars > 0 &&
                [...Array(emptyStars)].map((_, index) => (
                    <svg key={`empty-star-${index}`} width={starSize} height={starSize} viewBox="0 0 20 20">
                        <path
                            fill="#BBBAC0"
                            opacity="0.5"
                            d="M0 4C0 1.79086 1.79086 0 4 0H10V20H4C1.79086 20 0 18.2091 0 16V4Z"
